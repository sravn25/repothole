...
function Layout() {
  ...
  const [rStatus, setRepairStatus] = useState("");
  ...
  useEffect(() => {
    if (file) {
      if (outputClass === "potholes") {  // only save pothole's data to firebase
        ...
        const records = [
          {
            file: file, //image
            location: center.address,  //location address fetched
            reportDate: date,  //fetched date and time
            confidentialLevel: outputScore,  //need to get output from tensorflow.js
            repairStatus: "Under Review",  //default is set to "under review"
          },
        ];
        // create save record function
        // input = attributes set in line 61 
    ...
    >
      ...
      <Grid grow style={{ height: "100%", margin: 0 }}>
        ...
        <Grid.Col span={6}>
          ...
          />
          <Uploader sendOutput={handleOutput} sendLocation={retrieveLocation} sendDate={retrieveDate} sendFile={retrieveFile} />
        </Grid.Col>
